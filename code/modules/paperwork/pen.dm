...
/*
 * Процедура переименовывание конверта
 */

/obj/item/weapon/pen/proc/RenameEnvelope(mob/user as mob, obj/item/weapon/storage/envelope as obj)
	if ( !user || !envelope )
		return
	var/n_name = input(user, "What would you like to label the [envelope.name]?", "Envelope Labelling", null)  as text
	n_name = copytext(n_name, 1, 32)
	if(( get_dist(user,envelope) <= 1  && user.stat == 0))
		envelope.name = "envelope [(n_name ? text("- '[n_name]'") : null)]"
